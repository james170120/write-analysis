        ...
        if (field && inputElement) {
            ...
            if (fontSize !== null) {
                if (rawValue.length >= 6 && fontSize === 10) {
                    field.setFontSize(7);  // 6字以上公司名自動縮小到 7pt
    ...
    try {
        ...
        document.getElementById('templateSelect').addEventListener('change', (e) => {
            ...
            if (val === 'motorcycle') {
                ...
                // --- 👤 人壽險範本 ---
                // 注意：這裡不需要填寫 t_car_type (車輛種類)，因為最上面已經統一清空了
                presetChecks = [
                    'c_cat_life',     // 報告書：人身保險
                    'c_need_1',       // 需求：保障需求 (你可以依需求增減)
                    'c_type_1',       // 險種：壽險
                    'c_spec_no',      // 指定公司：否
                    'c_pay_2',        // 繳別：年繳 (通常壽險年繳居多，可自行修改)
                    'c_ret_4',        // 剩餘年期：21-30年
                    'c_src_1',        // 來源：薪資
                    'c_prov_1',       // 提供：保險公司網站
                    'c_prod_1',       // 商品：詳建議書 (壽險通常看建議書)
                    'c_term_1',       // 條款：依保險公司說明
                    'c_cov_1',        // 保障範圍：詳建議書
                    'c_rsn_1',        // 理由：保障完整
                    'c_aml_1',        // 洗錢地域 - 一般
                ...
                presetChecks = [
                    'c_cat_prop',     // 報告書類別：財產保險
                    'c_need_1',       // 投保目的：保障需求
                    'c_need_4',       // 投保目的：損害填補
                    'c_need_6',       // 投保目的：風險移轉
                    'c_type_7',       // 欲投保險種：責任險
                    'c_type_9',       // 欲投保險種：火災地震險
                    'c_spec_no',      // 指定保險公司：否
                    'c_pay_2',        // 繳費方式：年繳
                    'c_ret_7',        // 退休剩餘年期：短年期不適用
                    'c_src_6',        // 保費來源：營業收入 (太陽能電廠多為此來源)
                    'c_prov_1',       // 概況提供：保險公司網站
                    'c_prod_2',       // 保險商品：詳報價單/要保書
                    'c_term_1',       // 條款審閱：依保險公司提供說明
